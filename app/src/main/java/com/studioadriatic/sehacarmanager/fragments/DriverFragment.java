...
public class DriverFragment extends ListFragment {
    ...
    protected void setFragmentTitle() {
        ((MainActivity) getActivity()).setActionBarTitle(getString(R.string.drivers));
    }

    @Override
    protected void loadListData() {
        String token = App.getCurrentToken(getContext());
        Api.get(Api.LIST_DRIVERS, Api.getRequestParams(token), new AsyncHttpResponseHandler() {
            @Override
    ...
    @OnClick(R.id.fab)
    public void onClick() {
        getFragmentManager().beginTransaction().replace(R.id.content, new NewDriverFragment()).addToBackStack(TAG).commitAllowingStateLoss();
    }
}
